...
public class RabbitProxyManagementService {
    ...
    public static final String PROXY_RESPONSE_QUEUE_NAME = "proxy-response";
    private static final String PROXY_NAME = "proxy";
    private static final String ROUTING_KEY_REQUEST_PREFIX = "request.";
    private static final String ROUTING_KEY_RESPONSE_WITH_WILDCARD = "response.*";
    ...
    public OperationResult addProxy(String proxyName, String vhost, User user) throws RabbitConfigurationException {
        logger.info("=== Adding proxy: {}, vhost: {} ===", proxyName, vhost);
        OperationResult stats = new OperationResult();
        rabbitApiService.createUserIfNotExist(user, stats);
        ...
        rabbitApiService.createUserRightsForProxy(vhost, proxyName, user.username(), stats);
        rabbitApiService.createBindingIfNotExist(PROXY_NAME, ROUTING_KEY_REQUEST_PREFIX + proxyName, proxyName, vhost, stats);
        rabbitApiService.createBindingIfNotExist(PROXY_NAME, ROUTING_KEY_RESPONSE_WITH_WILDCARD, PROXY_RESPONSE_QUEUE_NAME, vhost, stats);
